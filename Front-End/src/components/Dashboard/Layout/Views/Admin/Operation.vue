...
export default {
  ...
  methods: {
    ...
    async update_ban(data) {
      const result = await this.$callapi.callAPIHandler(this.axios.patch(this.$api.operation.edit, {
        id: data.id,
        ban: data.ban
      }))
      if (result.data) {
        this.$notify({
          message: `${data.ban ? 'Ban' : 'Unban'} operation success`,
          type: 'success'
        })
      }
    },
