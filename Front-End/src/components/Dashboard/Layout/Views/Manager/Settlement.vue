    ...
    <card>
      ...
      <div class="row" style="padding: 15px">
        ...
        <div class="row col-sm-2 ml-auto">
          <p-button type="info" icon class="col-sm-1" @click="modal_add.status = true"><i
...
export default {
  ...
  sockets: {
    update_settlement(data) {
      ...
      if (findIndex >= 0) {
        ...
        this.data[findIndex].fee = data.fee
  ...
  methods: {
    ...
    async getLog(id) {
      const Result = await this.$callapi.callAPIHandler(this.axios.post(this.$api.settlement.log, {id}))
      if (Result.data) {
        this.log.modal = true
    ...
    async updateStatus(data) {
      const Result = await this.$callapi.callAPIHandler(this.axios.patch(this.$api.settlement.update, data))
      if (Result.data) {
        this.$notify({
